# Kiro × agora-code — insights, what works, and what to build next

**Purpose:** This is internal design memory for evolving the **Kiro-first** line of work (the experience you get when MCP + steering + `.kiro/hooks` are wired to `agora-code`). Use it to **plan improvements**, **onboard**, and **decide what to document next** — not as marketing copy.
...
- **`docs/planning/`** — **agora-code-kiro** only: Kiro IDE inputs + product direction (not shipped user docs).
...
| Layer | Role | Status |
|--------|------|--------|
| **MCP `agora-memory`** | Exposes DB + tools (`summarize_file`, `index_file`, `store_learning`, …) to the agent | **Core** — same server other editors use; Kiro is “just” another MCP client. |
...
| **`memory.db` + project scoping** | Git-remote–keyed memory separate from Kiro’s chat files | **Clear split of concerns** — Kiro owns chat UX; agora-code owns cross-session **structured** memory. |
...
| **`askAgent` (Ask Kiro)** | `agora-summarize-interaction`; the three `agora-summarize-before-read*`; `agora-index-after-*` (write / strreplace / editcode); `agora-log-grep-results`; `agora-index-on-save` | **9** |

So **today** the Kiro pack is **not** shell-only: anything that needs **`summarize_file` / `read_file_range` / `index_file` / `log_search` / `store_learning` over MCP** is implemented as **Ask Kiro** instructions, because the hook JSON does not invoke `agora-code summarize` / `agora-code index` with a file path.

**CLI already exists for a shell-first future:** `agora-code inject` and `agora-code checkpoint` (see `agora_code/cli.py`); `agora-code summarize <path> [--json-output]` and `agora-code index <path>` (same module). Claude-style pre-read uses `--json-output`; the same primitive could drive Kiro **if** Kiro exposes the target path (or JSON on stdin) to shell hooks — that contract is what **`docs/KIRO_ENV_CONTRACT.md`** is for once we know it.

**If your product direction is “shell first + tokens/context”:** you are aiming to **replace as many of the 9 Ask Kiro hooks as possible** with `runCommand` lines that call `agora-code …`. **Blockers today:** (1) per-event **file path** (and for grep, query + paths) into the shell; (2) **interaction summary** — a one-sentence `store_learning` is inherently LLM-shaped unless you drop it or pipe fixed text from somewhere else.

### 2.2 “Shell = cheap / Ask Kiro = credits + compliance” — elaborated

- **Shell:** Kiro runs a subprocess; **no second agent loop** → **no extra LLM credits** for that step. Success path: **stdout** is appended to the agent context (per Kiro’s hook behavior). Our shell hooks only run `agora-code inject --quiet` or `agora-code checkpoint --quiet` — both are **deterministic** and write/read project + DB via `session.py` / vector store.
...
**Token / context angle:** The **main token win** in this stack is **summarize → read range**, not the shell hooks. Steering explicitly tells the model to call `summarize_file` then `read_file_range` (`.kiro/steering/agora-memory.md`). The MCP tool descriptions in `agora_code/memory_server.py` reinforce the same. **Shell `inject`** then adds a **bounded** text block: `_build_recalled_context()` in `agora_code/session.py` is documented there as targeting **~350 tokens** for the structured inject sections (checkpoint, learnings, git, symbols, etc. — see the docstring and implementation starting ~line 576).

### 2.3 “Steering must be non-empty” — elaborated

**What steering is in our tree:** `.kiro/steering/agora-memory.md` starts with YAML frontmatter `inclusion: always` so Kiro **always** merges this file into the agent’s instructions.

**What it does:** It names **`agora-memory`**, lists which hooks run and at what **cost** (free vs small), gives the **CRITICAL** block for `agora-summarize-interaction` (exactly one `store_learning` then stop), and rules like “don’t call `get_session_context` every prompt — inject already loaded context” and “before reading any file, `summarize_file` then `read_file_range`”.
...
- **Kiro** persists **conversation UI state** under its app data (see `docs/KIRO_PLATFORM_AND_AGORA_PLAN.md` — e.g. `*.chat` under `kiro.kiroagent`). That is **opaque to agora-code**, not keyed the same way as our **project_id** (git remote), and not designed for **structured** recall (learnings, checkpoints, symbol index, file snapshots).
- **agora-code** stores **long-lived project memory** in **`~/.agora-code/memory.db`** (override with `AGORA_CODE_DB`) plus **active working state** in **`.agora-code/session.json`** (see header comments in `agora_code/session.py`). **`inject`** composes context from the **DB + git** via `_build_recalled_context()` — checkpoints (tagged learnings), recent learnings, git log, uncommitted files, symbol hints — not from Kiro’s chat files.
...
2. **`agora-auto-checkpoint.kiro.hook`** — `runCommand` → `agora-code checkpoint --quiet`.
...
**From the repo (we already have):** `agora_code/cli.py` (`inject`, `checkpoint`, `summarize`, `index`), `agora_code/session.py` (`_build_recalled_context`, session file layout), `agora_code/memory_server.py` (MCP tool schemas), `.kiro/hooks/*.kiro.hook`, `.kiro/steering/agora-memory.md`, `docs/DATABASE.md` + `DATABASE_AND_STRUCTURED_LAYER.md` for what the DB holds.
...
7. **PATH:** MCP subprocess often lacks full shell PATH → **full path to `agora-code`** in `mcp.json` is a recurring fix.
...
| **Two repos** | `agora-code` vs `kiro-ctx` — risk of **doc and hook drift** unless sync is intentional. |
...
**B. Cost / latency**
- Replace Ask Kiro **index** hooks with **shell** `agora-code index <path>` **if** Kiro documents stable env for touched path.
...
- Rule of thumb: **kiro-ctx** = Kiro-first narrative + experiments; upstream **agora-code** = shared core — decide what must stay identical (MCP contract, DB schema).
...
| `.kiro/steering/agora-memory.md` | Live agent rules | Version with hook changes; consider a **short “why”** comment block in a stub doc (steering file stays lean). |
| `power-agora-memory/POWER.md` | Distribution shape | Align with backlog **C** if you publish a Power. |
...
3. **`docs/SYNC_KIRO_CTX.md`** — one page: what must match between `kiro-ctx` and `agora-code`, how you merge, who owns README voice.

---

## 7. One-line summary

**Kiro gives chat + hooks + steering; agora-code gives a git-aware, queryable memory layer.** The Kiro-specific work is **gluing** those with minimal credits, **stable tool matching**, and **steering that matches hook behavior** — this file is where we **record what we learned** and **what to improve next**.
