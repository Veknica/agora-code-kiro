# Kiro user path through agora-code + context effects (investigation notes)
...
This file is grounded in the **current codebase**: entrypoint `agora-code` → `agora_code.cli:main` (`pyproject.toml` `[project.scripts]`), with Kiro wiring under `.kiro/`.

---

## 1. How a Kiro user actually uses agora-code

### One-time (per machine / project)

1. **`pip install agora-code`** (or editable install from a clone) so the **`agora-code`** binary exists. Prefer a **full path** in MCP config if Kiro’s subprocess has a thin `PATH`.
2. **MCP:** Add `.kiro/settings/mcp.json` (or `~/.kiro/settings/mcp.json`) so Kiro starts:
   - `command`: `agora-code` (or full path)
   - `args`: `["memory-server"]`  
   That runs `memory_server()` in `agora_code/cli.py`, which calls `agora_code.memory_server.serve_memory` — stdio MCP exposing tools such as `get_session_context`, `store_learning`, `summarize_file`, `read_file_range`, `index_file`, `log_search`, etc. (the `memory-server` docstring in `cli.py` lists a subset; the server registers more in `memory_server.py`).
3. **Steering:** Copy `.kiro/steering/agora-memory.md` — `inclusion: always` so the agent is instructed **when** to use MCP and how (e.g. summarize before read, stop after one `store_learning` on agent stop).
4. **Hooks:** Copy `.kiro/hooks/*.kiro.hook` — see §2.

### Every session / turn (automatic)

- **Prompt submit:** shell hook runs **`agora-code inject --quiet`** → `inject()` in `cli.py` → `_build_recalled_context()` in `agora_code/session.py` builds text from **SQLite + git** (not from Kiro chat files). Stdout is appended to the agent context.
- **Agent stop:** (1) Ask Kiro hook → model calls **`store_learning`** via MCP; (2) shell hook → **`agora-code checkpoint --quiet`** → `checkpoint()` / `update_session()` in `cli.py` / `session.py` (session JSON + dual-write to DB where applicable).
...
### Optional manual (same as any agora-code user)

From Kiro’s terminal or any shell in the repo:

- `agora-code status`, `agora-code memory`, `agora-code list-learnings`, `agora-code recall "…"`, `agora-code complete --summary "…"`, etc. — all live under **`agora_code/cli.py`** as Click subcommands.

### What a Kiro user typically *does not* touch

- **`agora-code scan` / `serve` / `agentify`** — API-route / workflow features; orthogonal to the **memory** path unless they use those product lines too.
...
|-------------|-----------|------------|
| **`runCommand`** | Subprocess string, e.g. `agora-code inject --quiet` | `.kiro/hooks/*.kiro.hook` → `then.command` |
| **`askAgent`** | Prompt to the model; model calls **MCP tools** | Same JSON files + steering; MCP server is **`agora-code memory-server`** from `mcp.json` |

There is **no** separate “Kiro CLI” — only **`agora-code`** plus Kiro’s Hook UI / JSON. Versioning = git on `.kiro/` + global `~/.kiro/` if used.
...
| **Kiro’s own chat transcript** | Still grows in Kiro’s UI/storage; agora-code **doesn’t replace** that — it adds **structured memory beside** it. |
...
| Area | File(s) |
|------|---------|
| CLI entry + subcommands | `agora_code/cli.py` (`main`, `inject`, `checkpoint`, `summarize`, `index`, `memory-server`, …) |
| Package script name | `pyproject.toml` → `agora-code = "agora_code.cli:main"` |
| MCP tool handlers | `agora_code/memory_server.py` |
| Inject / session / checkpoint persistence | `agora_code/session.py` (`_build_recalled_context`, `update_session`, …) |
| DB / learnings | `agora_code/vector_store.py` (+ `docs/DATABASE.md`) |
| Kiro JSON | `.kiro/settings/mcp.json`, `.kiro/hooks/*.kiro.hook`, `.kiro/steering/agora-memory.md` |

---

## 6. Full read-through: `cli.py` + `memory_server.py` (no skim)

**Verified:** `agora_code/memory_server.py` (1006 lines) and `agora_code/cli.py` (2879 lines) were read **line-by-line** in order for this section (Mar 2026). Below is the **exact** Kiro-relevant command flow and one important split people often confuse.
...
| `checkpoint` | Merges flags into `session.update_session` → `.agora-code/session.json` + `save_session` to DB. |
...
|------|------|---------------------|
| **`agora-code inject`** | `cli.inject` → `_build_recalled_context()` only | **Shell hook** on every prompt |
